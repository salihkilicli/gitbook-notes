...
> In [statistics](https://wiki2.org/en/Statistics) and [machine learning](https://wiki2.org/en/Machine\_learning), the **bias–variance tradeoff** is the property of a model that the [variance](https://wiki2.org/en/Variance) of the parameter estimates across [samples](https://wiki2.org/en/Sample\_\(statistics\)) can be reduced by increasing the [bias](https://wiki2.org/en/Bias\_of\_an\_estimator) in the [estimated](https://wiki2.org/en/Estimation\_theory) [parameters](https://wiki2.org/en/Statistical\_parameter). The **bias–variance dilemma** or **bias–variance problem** is the conflict in trying to simultaneously minimize these two sources of [error](https://wiki2.org/en/Errors\_and\_residuals\_in\_statistics) that prevent [supervised learning](https://wiki2.org/en/Supervised\_learning) algorithms from generalizing beyond their [training set](https://wiki2.org/en/Training\_set). - Wikipedia
...
This part of the notes consists of advice given by Andrew Ng which you can find in his book [Machine Learning Yearning](https://www.deeplearning.ai/machine-learning-yearning/) or on his ML Course (one of my favs)[ @ Coursera](https://www.coursera.org/learn/machine-learning?campaignid=685340575\&adgroupid=52515609594\&device=c\&keyword=machine+learning+mooc\&matchtype=b\&network=g\&devicemodel=\&adpostion=\&creativeid=243289762946\&hide\_mobile\_promo=\&gclid=Cj0KCQjw8fr7BRDSARIsAK0Qqr457L47D0puHVHlN3g12J68dQ702DwuPMT5fLIEavktONZVExgOhzAaAqn6EALw\_wcB).
...
1. Clustering (K-Means, DBSCAN, Hierarchial Clustering Analysis (HCA), etc.)
2. Dimensionality reduction (PCA, t-SNE, etc.)
3. Anomaly & Novelty detection (DBSSCAN, Local Outlier Factor (LOF), Isolation forest, etc.)
...
* Clustering algorithms such as K-Means variations or DBSCAN
...
* It is the type of learning system in which the system itself is capable of learning incrementally, i.e. the algorithm is being fed data sequentially, either individually (**batch**) or in smaller groups (**mini-batches**). Due to its speed and (computing) cost friendliness, the system can learn about the instance on the fly.
...
* A model parameter is the parameter model that tries to optimize to minimize the given metric such as cost function. A learning algorithm's hyperparameter is a parameter that belongs to the algorithm that helps optimize its performance rather than the model itself.
...
* Test set is a part of the data (most of the time chosen from 10-30% of the data randomly) and it is used to evaluate the model's performance before being put into production
...
* This is a tricky question. Andrew Ng explains this concept very clearly in his course. Here is the answer from the book:
